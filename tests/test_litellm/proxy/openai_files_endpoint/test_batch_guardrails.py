...
async def test_scan_does_not_mutate_the_parsed_record():
    ...
    record = _ParsedRecord(line_number=1, payload=payload, raw=json.dumps(payload))
...
def _blocking(needle, status_code=400):
    def _hook(data):
        for message in data.get("messages") or []:
            if isinstance(message.get("content"), str) and needle in message["content"]:
                raise HTTPException(status_code=status_code, detail={"error": "Violated guardrail policy"})
...
async def test_redact_mode_keeps_a_masked_record_instead_of_rejecting():
    ...
    rewritten = json.loads(result.changes[0].record)
...
async def test_every_status_litellm_calls_a_block_drops_the_record(status_code):
    ...
    assert result.changes == (RecordDropped(line_number=2, custom_id="b"),)
...
async def test_report_names_every_changed_record_in_file_order():
    ...
    assert [(r.line, r.custom_id, r.action) for r in report.modified_records] == [
        (2, "b", "dropped"),
        (3, "c", "redacted"),
    ]
...
async def test_raising_a_native_block_exception_drops_whatever_status_it_carries():
    ...
    def _hook(data):
        if "tripwire" in data["messages"][0]["content"]:
            raise GuardrailRaisedException(guardrail_name="g", message="refused", status_code=503)

    result = await _scan_full(_jsonl(_record("b", content="tripwire")), FakeProxyLogging(_hook))

    assert result.changes == (RecordDropped(line_number=1, custom_id="b"),)
...
async def test_a_redacted_record_keeps_its_own_body_metadata():
    ...
    result = await _scan_full(_jsonl(record), FakeProxyLogging(_redact_containing("secret")))

    rewritten = json.loads(result.changes[0].record)
    assert rewritten["body"]["metadata"] == {"team": "finance"}
    assert rewritten["body"]["messages"][0]["content"] == "my *** is here"
    assert "litellm_metadata" not in rewritten["body"]
